...
struct biquad_cascade {
    int16_t history[3*2+2];
    int16_t saved_fraction[3];
};

static int16_t filter(int16_t input, struct biquad_cascade* state, const struct biquad_coeffs* coeffs) {
    int i;
    int16_t output, frac;

    for (i = 0; i != 3; ++i) {
        int32_t acc = state->saved_fraction[i];
...
static void stopband_test(void) {
    struct biquad_cascade cascade = {0};
    double t = 0.0;
    const double fs = 1000;
    const double f = 200;
    while (t < 0.3) {
        int16_t x = (int16_t)(cos(2*M_PI*f*t) * (1<<Q));
        int16_t y = filter(x, &cascade, coeffs);
        printf("%d,%d\n", x, y);
        t += 1/fs;
    }
}
